...
public class ZipkinConfig {
    ...
    public SpanCollector spanCollector() {
        HttpSpanCollector.Config config = HttpSpanCollector.Config.builder().compressionEnabled(false).connectTimeout(5000)
                .flushInterval(3).readTimeout(6000).build();
        return HttpSpanCollector.create("http://182.254.197.194:9411/", config, new EmptySpanCollectorMetricsHandler());
    }
    ...
    public Brave brave(SpanCollector spanCollector) {
        Brave.Builder builder = new Brave.Builder("sun-service");// 指定serviceName
